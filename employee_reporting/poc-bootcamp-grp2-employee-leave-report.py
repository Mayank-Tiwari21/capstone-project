...
date_range = spark.sql(f"SELECT explode(sequence(to_date('{current_date_str}'), to_date('{end_of_year_str}'))) AS date")
...
print(f"Total working days from {current_date_str} to {end_of_year_str}: {total_working_days}")

future_leaves = leave_df \
    .filter((col("leave_date") >= lit(current_date_str)) & (col("leave_date") < lit(end_of_year_str))) \
...
# =====================used to store the emp_id ,,,and all the upcoming leaves===============

# flagged_emp_ids = flagged.filter(col("flagged") == "Yes").select("emp_id").distinct()

# filtered_future_leaves = future_leaves.join(flagged_emp_ids, on="emp_id", how="inner")

# upcoming_leave_table = filtered_future_leaves.groupBy("emp_id") \
#     .agg(sort_array(collect_list("leave_date")).alias("upcoming_leaves")) \
#     .withColumn("leave_date", explode("upcoming_leaves")) \
#     .select("emp_id", "leave_date")

# upcoming_leave_table.withColumn("run_date", lit(today_str)) \
#     .write.mode("overwrite").partitionBy("run_date") \
#     .parquet("s3://poc-bootcamp-capstone-project-group2/gold/employee_leaves_date_info/")

# ================used to store the emp_id and upcmong leaves_cnt====================
