...
with open("/home/ubuntu/spark-codes/data/vocab.json") as f:
    vocab = set(json.load(f))
with open("/home/ubuntu/spark-codes/data/marked_word.json") as f:
...
def func_strike_calculator(employee_strikes_df,employee_flags_count_df ):
    ...
    for i in range(1, 11):
        ...
        updated_df = updated_df.withColumn(
            f"strike_{i}",
            when(
                (col("strike_count")>=i),salary_after_i_strikes
            ).otherwise(lit(None))
        )
...
query = kafka_df.writeStream \
    ...
    .outputMode("update") \
    .option("checkpointLocation", "/home/ubuntu/kafka-checkpoint/") \
